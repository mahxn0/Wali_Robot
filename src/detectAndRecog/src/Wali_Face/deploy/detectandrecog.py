                        ...
                        for j, chip in enumerate(chips):
                            ...
                            for i,key in enumerate(sign_feature):
                                ...
                                #计算两张图片特征的差
                                    ...
                                    if dist<=1.2:
                                        print(key)
                                        #RecogResults.append((key,xz,yz))
                                        cv2.putText(drawimg,key, (xz,yz), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255),4)
                                    elif dist>1.2:
                                        coptime+=1
                            if coptime==len(sign_feature):
                                cv2.putText(drawimg,"others", (xz,yz), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255),4)

                    else:
                        continue
                #for RecogResult in RecogResults:
                #    key=RecogResult[0]
                #    xz=RecogResult[1]
                #    yz=RecogResult[2]
