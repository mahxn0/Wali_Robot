...
#define DEFAULT_SESSION_PARA \
	 "sub = iat, domain = iat, language = zh_cn, accent = mandarin, sample_rate = 16000, result_type = json, result_encoding = gb2312"
...
static void end_sr_on_vad(struct speech_rec *sr)
{
	...
	if (sr->rec_stat == MSP_REC_STATUS_COMPLETE && !rslt) {
...
int sr_init_recorder(struct UserData * user_data, const char * session_begin_params, struct speech_rec * sr) {
	int errcode;
	WAVEFORMATEX wavfmt = DEFAULT_FORMAT;

	errcode = create_recorder(&user_data->recorder, iat_cb, (void*)sr);
	...
	errcode = open_recorder(user_data->recorder, get_default_input_dev(), &wavfmt);
	if (errcode != 0) {
	...
	return 0;
fail:
	if (user_data->recorder) {
		destroy_recorder(user_data->recorder);
		user_data->recorder = NULL;
	}
	return errcode;
}
int start_recorder(struct UserData * user_data) {
	if (user_data->recorder) {
		int ret;
		ret = start_record(user_data->recorder);
		if (ret != 0) {
...
int stop_recorder(struct UserData * user_data) {
	if (user_data->recorder) {
		int ret;
		ret = stop_record(user_data->recorder);
		if (ret != 0) {
			sr_dbg("Stop failed! \n");
			return -E_SR_RECORDFAIL;	
		}
		wait_for_rec_stop(user_data->recorder, (unsigned int)-1);
		return 0;
	}
	return -E_SR_RECORDFAIL;	
}
...
{
	struct UserData *grm_data = (struct UserData *)udata;
	...
	if (MSP_SUCCESS == ecode && NULL != info) {
		if (NULL != grm_data)
			snprintf(grm_data->grammar_id, 32 - 1, info);
	}
	else
		printf("构建语法失败！%d %s\n", ecode, info);
...
int sr_build_grammar(const char * grammar_type, const char * grammar_path, 
			const char * session_begin_params,	struct UserData * user_data)
{
	int ret;
	FILE *grm_file						= NULL;
	...
	grm_file = fopen(grammar_path, "rb");
	if (NULL == grm_file) {
		sr_dbg("%s open file ! error: %s\n", grammar_path, strerror(errno));
	...
	ret = QISRBuildGrammar("bnf", grm_content, grm_cnt_len, session_begin_params, build_grm_cb, user_data);
	if (ret != MSP_SUCCESS) 
	{
		sr_dbg("ret=%d, grammar_type == %s, grm_content == %s, grm_cnt_len == %d, session_begin_params == %s, build_grm_cb == %s, user_data == %s \n", ret, grammar_type, grm_content,grm_cnt_len, session_begin_params, build_grm_cb, user_data);
	}
...
int sr_init(struct speech_rec * sr, const char * session_begin_params, struct speech_rec_notifier * notify)
{
int errcode;
	size_t param_size;
...
void sr_uninit_recorder(struct UserData * user_data)
{
	if (user_data->recorder) {
		if(!is_record_stopped(user_data->recorder))
			stop_record(user_data->recorder);
		close_recorder(user_data->recorder);
		destroy_recorder(user_data->recorder);
		user_data->recorder = NULL;
	}
}
...
// int sr_start_listening(struct speech_rec *sr, struct UserData * user_data)
// {
// 	int ret;
// 	const char*		session_id = NULL;
// 	int				errcode = MSP_SUCCESS;
